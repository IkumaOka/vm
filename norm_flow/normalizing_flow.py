...
class PlanarFlow:
    def __init__(self, dim):
        ...
        print(torch.tensor(tf.random.truncated_normal(shape=(1, self.dim))))
        self.w = torch.tensor(tf.random.truncated_normal(shape=(1, self.dim)))
        self.b = torch.zeros(1)
        self.u = torch.tensor(tf.random.truncated_normal(shape=(1, self.dim)))
